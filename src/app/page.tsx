...
function MapsScreen() {
  ...
  const [shareTarget, setShareTarget] = useState<MindMap | null>(null);
  ...
  const myEmail = () => getFirebaseAuth().currentUser?.email?.toLowerCase() ?? null;

  const load = useCallback(async () => {
    try {
      // Instant render from cache; the server response below stays authoritative.
      listMapsFromCache(uid(), myEmail()).then((cached) => {
        if (cached) setMaps((current) => current ?? cached);
      });

      const data = await listMaps(uid(), myEmail());
              ...
              {renaming?.id === map.id ? (
                ...
                <>
                  ...
                  >
                    ...
                    ) : (map.member_emails?.length ?? 0) > 0 ? (
                      <span className="mt-1.5 inline-block rounded-full bg-[var(--accent-soft)] px-2 py-0.5 text-[10px] font-semibold text-[var(--accent)]">
                        Compartilhado com {map.member_emails!.length} pessoa(s)
                      </span>
                    ) : null}
                  </button>
          ...
          >
            ...
            {(shareTarget.member_emails?.length ?? 0) > 0 && (
              <ul className="mt-3 space-y-1.5">
                {shareTarget.member_emails!.map((em) => (
                  <li
                    key={em}
                    className="flex items-center justify-between rounded-lg border border-[var(--line)] px-3 py-1.5 text-sm"
                  >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setShareBusy(true);
                        try {
                          const next = await setMapMembers(
                            shareTarget.id,
                            shareTarget.member_emails!.filter((x) => x !== em)
                          );
                          setShareTarget({ ...shareTarget, member_emails: next });
                          await load();
                        } catch (e) {
                          setError('Falha ao remover: ' + (e instanceof Error ? e.message : ''));
                        }
                        setShareBusy(false);
                      }}
            ...
            <form
              ...
              onSubmit={async (e) => {
                e.preventDefault();
                const em = shareEmail.trim().toLowerCase();
                if (!em || !em.includes('@')) return;
                setShareBusy(true);
                try {
                  const next = await setMapMembers(shareTarget.id, [
                    ...(shareTarget.member_emails ?? []),
                    em,
                  ]);
                  setShareTarget({ ...shareTarget, member_emails: next });
                  setShareEmail('');
                  ...
                  setError('Falha ao compartilhar: ' + (e2 instanceof Error ? e2.message : ''));
                }
                setShareBusy(false);
              }}
            >
              ...
              <button
                type="submit"
                disabled={shareBusy || !shareEmail.includes('@')}
                className="shrink-0 rounded-lg bg-[var(--ink)] px-3 py-2 text-sm font-medium text-[var(--bg)] hover:opacity-90 disabled:opacity-50"
              >
                Adicionar
              </button>
            </form>
